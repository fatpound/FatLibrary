...
namespace fatpound::bit::details
{
    ...
    inline constexpr auto Mask_(const T& endBit, const T& startBit) -> T
    {
        if constexpr (ForOne)
        {
            if (startBit > endBit)
...
namespace fatpound::bit
{
    ...
    inline constexpr auto OneMask(const T& endBit, const T& startBit) -> T
    {
    ...
    /// 
    template <std::unsigned_integral T = std::size_t>
    inline constexpr auto ZeroMask(const T& endBit, const T& startBit) -> T
    {
