...
namespace fatpound::algorithm
{
    ...
    static constexpr void CountingSort_Unchecked(std::vector<T>& inVec, std::vector<T>& outVec, const std::unsigned_integral auto& maxValue) noexcept
    {
    ...
    template <std::unsigned_integral T>
    static constexpr void CountingSort(std::vector<T>& inVec, std::vector<T>& outVec)
    {
        if (inVec.empty())
        {
