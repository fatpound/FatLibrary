...
namespace fatpound::file
{
    ...
    /// 
    static auto NameAndExtension    (const std::filesystem::path& path) -> std::pair<std::string, std::string>
    {
...
#undef FATLIB_FILE_LINECOUNT_DIR_LOOP_GENERATOR
    ...
    /// 
    static auto LineCount_Unchecked (const std::filesystem::path& path) -> std::size_t
    {
    ...
    /// @return The number of lines in the file
    /// 
    static auto LineCount           (const std::filesystem::path& path) -> std::size_t
    {
        if (not std::filesystem::exists(path))
        {
            throw std::runtime_error("The given path does NOT exist!");
    ...
    /// 
    static auto LineCount_Dir       (const std::filesystem::path& path, const bool& recurse = false, const std::vector<std::string>& extensionFilter = {}) -> std::size_t
    {
    ...
    /// 
    static void EncryptDecrypt_Dir  (const std::filesystem::path& inPath, const std::size_t& key, std::filesystem::path outPath = {}, const bool& recurse = false, const std::vector<std::string>& extensionFilter = {})
    {
        namespace fs = std::filesystem;
